...
from torchvision.models import ResNet18_Weights, VGG13_BN_Weights
...
def cotrain(net1,net2, labeled_loader, unlabled_loader, 
            optimizer1_1, optimizer1_2, optimizer2_1, optimizer2_2, 
            scheduler1_1, scheduler2_1, scheduler1_2, scheduler2_2, criterion):
...
####################
        ...
        train_loss1_1 = criterion(outputs1_1, targets)
        train_loss1_1.backward()
        train_loss2_1 = criterion(outputs2_1, targets)
        train_loss2_1.backward()
        optimizer1_1.step()
        optimizer2_1.step()
        scheduler1_1.step()
        scheduler2_1.step()

        train_loss += train_loss1_1.item()
        _, predicted1_1 = outputs1_1.max(1)
        _, predicted2_1 = outputs2_1.max(1)
        total += targets.size(0)
        correct += predicted1_1.eq(targets).sum().item()
...
####################
        ...
        outputs1_2 = net1(inputs)
        outputs2_2 = net2(inputs)
        _, predicted_1_2 = torch.max(outputs1_2, 1)
        _, predicted_2_2 = torch.max(outputs2_2, 1)

        agree = predicted_1_2.eq(predicted_2_2)
        pseudo_label = torch.where(agree, predicted_1_2, torch.tensor([-1]).cuda())

        mask = pseudo_label != -1
        pseudo_inputs = inputs[mask]
        pseudo_label = pseudo_label[mask]

        outputs1_2 = net1(pseudo_inputs)
        outputs2_2 = net2(pseudo_inputs)
        train_loss1_2 = criterion(outputs1_2, pseudo_label)
        train_loss1_2.backward()
        train_loss2_2 = criterion(outputs2_2, pseudo_label)
        train_loss2_2.backward()
        optimizer1_2.step()
        optimizer2_2.step()
        scheduler1_2.step()
        scheduler2_2.step()
...
if __name__ == "__main__":
    ...
    optimizer1_1 = optim.Adam(model1.parameters(), lr=0.001, betas=(0.9, 0.999), eps=1e-08, weight_decay=0, amsgrad=False)
    #Optimizer for model 1 in labeled training
    optimizer2_1 = optim.Adam(model2.parameters(), lr=0.001, betas=(0.9, 0.999), eps=1e-08, weight_decay=0, amsgrad=False)
    #Optimizer for model 2 in labeled training 

    optimizer1_2 = optim.Adam(model1.parameters(), lr=0.001, betas=(0.9, 0.999), eps=1e-08, weight_decay=0, amsgrad=False)
    #Optimizer for model 1 in unlabeled training
    optimizer2_2 = optim.Adam(model1.parameters(), lr=0.001, betas=(0.9, 0.999), eps=1e-08, weight_decay=0, amsgrad=False)
    #Optimizer for model 2 in unlabeled training

    epoch = 400 #Input the number of epochs
    
    scheduler1_1 = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer1_1, T_0=350, T_mult=2, eta_min=0)
    scheduler2_1 = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer2_1, T_0=350, T_mult=2, eta_min=0)
    scheduler1_2 = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer1_2, T_0=350, T_mult=2, eta_min=0)
    scheduler2_2 = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer2_2, T_0=350, T_mult=2, eta_min=0)
    ...
    if args.test == 'False':
        ...
        for e in range(0, epoch):
            cotrain(model1, model2, labeled_loader, unlabeled_loader, 
                    optimizer1_1, optimizer1_2, optimizer2_1, optimizer2_2, 
                    scheduler1_1, scheduler2_1, scheduler1_2, scheduler2_2, criterion)
